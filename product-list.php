...
<?php
    if(!empty($_POST['Search'])){
        ...
        $q="SELECT * FROM product WHERE(prd_code LIKE '%".$Keyword."%' OR prd_name LIKE '%".$Keyword."%' ) ORDER BY prd_id ASC";  
      }else{

        $q="SELECT * FROM product ORDER BY prd_id ASC";  
      }
...
?>
		 ...
		 <a  href="main.php?stt=product-edit&ID=<?=$ID?>"> Edit</a> </td>
		  ...
		  <a href="<?php echo $Action; ?>?Table=product&Sql=Delete&ID=<?=$ID?>" onclick="return confirm('Confirm to delete info <?=$name?> Logout')">  Delete</a>		  </td>
        </tr>
