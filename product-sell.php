<div id="selection">
	...
	<form method="post" action="sell-sess.php" name="form1"  id="form1"  onsubmit="return chk_form()" enctype="multipart/form-data" class="form-horizontal">
	
		
	
	<div id="title-txt" style="margin-top: 10px; margin-bottom: 0px;">
		<label id="title-h2">Sales record</label> 
	</div>

	<?php if(count($_SESSION['sess_item_id'])>0){ ?>
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-condensed">
	<tr style="background:#f4f4f4;">
		<td width="10%" id="f-center">Product code</td>
		<td width="25%" id="f-left">Product list</td>
		<td width="10%" id="f-center">Size</td>
	...
	<?PHP	
		...
		$listc = count($_SESSION['sess_item_id']);
		for($i=0;$i<count($_SESSION['sess_item_id']);$i++){
			$sql = mysqli_query($con,"SELECT * FROM ".$Table." WHERE prd_id='".$_SESSION['sess_item_id'][$i]."'");
			$cols =1; //จำนวนแสดงรายการแบบคู่ หรือ แบบเดี่ยว
			$c = $cols; 
		$rs = mysqli_fetch_array($sql);
		include "table-sql/tb-product.php";
	
		$unit_price = $_SESSION['sess_item_num'][$i] * $prd_price;
		$total = $total + $unit_price;
		$unit_num = ($unit_num+$_SESSION['sess_item_num'][$i]);
		$code = sprintf("%05d",$ID);
	...
	<tr style="border-bottom: 1px solid #ddd;">
		...
		?>	
		<input <?=$disp?> name="txt_num[]" type="text" id="txt_num[]" style="width: 50px; height: 14px; text-align:center; border: 1px solid #ccc;" value="<?=$_SESSION['sess_item_num'][$i]?>" />
		
		</td>
		<td valign="middle" id="f-right"> 
		<?=number_format($prd_price,2)?>	</td>
		<td valign="middle" id="f-right"><?=number_format($unit_price,2)?></td> 
		<td valign="middle" id="f-center" style="padding-top: 6px;"><a class="btn-cart" href="sell-sess.php?sell=sess-del&Sess_id[]=<?=$ID?>"><i class="icon-trash"></i> </a></td>
		</tr>
		<?php 
	...
	<?php if(!empty($_GET['pay'])){ ?>
			<a type="button" class="btn" href='main.php?stt=product-sell' ><i class="fas fa-undo"></i>  Back </a>
			
			<a type="button" class="btn" href="main.php?stt=product-sell"><i class="fas fa-edit"></i>  Edit list</a>
<button type="button" class="btn" name="button" onclick="cal_price('<?php echo $total; ?>')"><i class="fas fa-calculator"></i> Calculate price</button>
			...
			<button type="submit" class="btn" name="sell" value="Confirm" onclick="return fcnConfirm()"><i class="fas fa-clipboard"></i> Sales record</button>
			
			
			
	<?php }else{ ?>
		
			<button type="submit" class="btn" name="sell" value="add-item" style="margin-left:5px;"><i class="fas fa-plus-circle"></i> Select more product</button>	
			<button type="submit" class="btn" name="sell" value="calculate"><i class="fas fa-cash-register"></i> Charge for product</button>	
	...
	<script language="javascript">
		function fcnConfirm(){
			return confirm("Confirm to Save Sales record ");
		}
		function fcnCancel(){
			return confirm("Cancel this sales record");//ต้องการยกเลิกการSavesell
		}
	</script>
	<?php }else{ ?>
		<div style="padding:10px; margin:5px;">
		<strong>No product list</strong>
			[ <a type="button" class="link" href="main.php?stt=product-list" style="margin-bottom: 10px;"> Select product</a> ]
		</div>
	<?php } ?>
