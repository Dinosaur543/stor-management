...
function page_navigator($before_p,$plus_p,$total,$total_p,$chk_page){     
    ...
    $lt_page=$total_p-4;  
	$linkp = "?stt=".$_GET['stt']."&ID=".$_GET['ID']."";
    if($chk_page>0){    
        echo "<a  href='".$linkp."&s_page=$pPrev&urlquery_str=".$urlquery_str."' class='naviPN'>Prev</a>";  
    }  
    if($total_p>=11){  
        if($chk_page>=4){  
    ...
    if($chk_page<$total_p-1){  
        echo "<a href='".$linkp."&s_page=$pNext&urlquery_str=".$urlquery_str."'  class='naviPN'>Next</a>";  
    }  
}     
?>